...
# For development: remove existing DB files so schema changes (like new columns) are applied fresh.
import os
project_root_db = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "college_app.db"))
backend_db = os.path.abspath(os.path.join(os.path.dirname(__file__), "college_app.db"))
for db_file in (project_root_db, backend_db):
    if os.path.exists(db_file):
        try:
            os.remove(db_file)
            print(f"Removed old DB to apply schema changes: {db_file} - seed.py")
...
for s in students_to_seed:
    ...
    if not db.query(models.Student).filter(models.Student.roll_no == s["id"]).first():
        db.add(models.Student(
            ...
            profile_pic=f"https://ui-avatars.com/api/?name={s['name']}&background=random"
        ))
